            ...
            },
{
	...
	var formattedLinkQuery =
	    'SELECT ?href {' +
	    '  BIND( <http://ldf.fi/nbf/'++'> as ?person)' +
	    '  ?person  nbf:formatted_link ?href .'+
	    '}';

	var sentenceQuery =
	    'SELECT * {' +
	    '  BIND( <file:///tmp/data/nlp/talousvaikuttajat/?iid=1> as ?person)' +
	...
	function getLenStatistics(facetSelections) {
        ...
        function getResults(facetSelections) {
            ...
            //return nbfEndpoint.getObjectsNoGrouping(qry).then(function(results) {
                ...
                //}
                var promises = {};
                var uposQry = sentenceQuery;//lemmaQry.replace(/<DOC>/g, '<' + _.map(results, 'id').join('> <') + '>');
